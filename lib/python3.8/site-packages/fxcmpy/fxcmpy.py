...
class fxcmpy(object):
    ...
    def open_trade(self, symbol, is_buy,
        ...
        if trailing_step is not None:
            try:
                trailing_step = float(trailing_step)      # DH change from float to int
            except:
    ...
    def create_oco_order(self, symbol, is_buy, is_buy2, amount, is_in_pips,
        ...
        try:
            trailing_stop_step = float(trailing_stop_step)        # DH replace float to int
        except:
            raise ValueError('trailing_stop_step must be a number.')

        try:
            trailing_stop_step2 = float(trailing_stop_step2)      # DH replace float to int
        except:
